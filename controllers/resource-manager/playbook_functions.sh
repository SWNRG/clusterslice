...
function create_playbook () {
   ...
   cp $playbook_path/$playbookname.template $playbook_path/$playbookname
   # update template
   input=$2[@]
   keyvalues=("${!input}")
   ...
   for keyvalue in ${keyvalues[@]}; do
      ...
      if [ `uname` == "Linux" ]; then
         sed -i "s $key $value g" $playbook_path/$playbookname
      else
         sed -i "" "s $key $value g" $playbook_path/$playbookname     
      fi
   done
   ...
   if [ `uname` == "Linux" ]; then
         sed -i "s {@.*@} none g" $playbook_path/$playbookname
   else
         sed -i "" "s {@.*@} none g" $playbook_path/$playbookname
   fi
}
...
function configure_server () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/configure_server.yaml #-c paramiko #-K
...
function install_kubernetes_base () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/install_kubernetes_base.yaml #-c paramiko
...
function install_kubernetes_master () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/install_kubernetes_master.yaml -c paramiko
...
function wait_for_cluster () {
 ...
 echo "Waiting for kubernetes cluster to be completed in host $node_name with username $admin_username."
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/wait_for_cluster.yaml -c paramiko
...
function install_kubernetes_worker () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/install_kubernetes_worker.yaml #-c paramiko
...
function distribute_file () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/distribute_file.yaml #-c paramiko
...
function wait_for_file () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/wait_for_file.yaml #-c paramiko
...
function install_application () {
  ...
  # execute playbook
  ansible-playbook $ansible_debug -i $hostsfile $playbook_path/install_$app_name.yaml -c paramiko
